# Setting up an INSPIRE Download service based on the OGC SensorThings API standard
...
* [7. INSPIRE Download Services based on STA](#inspire-sta)
    * [7.1. Main principles](#main-principles)
    * [7.2. Requirements class “INSPIRE-pre-defined-data-set-download-STA”](#req-pre-defined)
    * [7.3. Requirements class “INSPIRE-multilinguality”](#req-multilinguality)
    * [7.4. Requirements class “INSPIRE-STA-GeoJSON”](#req-sta-json)
    * [7.5. Requirements class "INSPIRE-bulk-download"](#req-bulk-download)
* [8. Bibliography](#bibliography)
* [Annex A: Abstract Test Suite](#ats)
* [Annex B: Mapping the requirements from the IRs to the OGC  API - Features standard (and extensions)](#ir2sta)
...
This document proposes a technical approach for implementing the requirements set out in the [INSPIRE Implementing Rules for download services](http://data.europa.eu/eli/reg/2009/976/oj) based on the [OGC SensorThings API standard](https://www.ogc.org/standards/sensorthings). 
...
In order to facilitate the use of off-the-shelf software implementing the OGC SensorThings API (STA) to meet the requirements in this document, INSPIRE-specific extensions are limited to the absolute minimum. Where several implementation options exist, this document describes the specific way of application of the STA and associated standards to meet the requirements of the INSPIRE Implementing Rules for download services.

### SensorThings API - a brief overview


## 2. Scope <a name="scope"></a>
This document proposes a technical approach for implementing the requirements set out in the [INSPIRE Implementing Rules for download services](http://data.europa.eu/eli/reg/2009/976/oj) based on the [OGC SensorThings API Part 1: Sensing standard](http://docs.opengeospatial.org/is/15-078r6/15-078r6.html). The approach described here is not legally binding and shows one of several ways of implementing INSPIRE Download services.

## 3. Conformance <a name="conformance"></a>
This specification defines the following requirements classes:

- [INSPIRE-pre-defined-data-set-download-STA (mandatory)](#req-pre-defined)
- [INSPIRE-multilinguality (conditional)<sup> 1</sup>](#req-multilinguality)
- [INSPIRE-STA-GeoJSON (optional)](#req-sta-json)
...
- **[STA](http://docs.opengeospatial.org/is/15-078r6/15-078r6.html)** - OGC SensorThings API - Part 1: Sensing<sup> 2</sup>
...
| feature | Abstraction of real world phenomena. **NOTE** The concept of a `feature` is synonymous to a `spatial object` in INSPIRE | [STA](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_feature) |
| feature collection | A set of features from a data set. | [STA](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_feature_collection)
...
| STA | OGC SensorThings API standard |
| URL |	Uniform Resource Locator |

## 7. INSPIRE Download Services based on STA <a name="inspire-sta"></a>
This section describes the requirements a Web API shall fulfill in order to be compliant with both ‘OGC SensorThings API’ and INSPIRE requirements for download services.
...
For example, two data sets (with their own metadata records), one on air qualiy observations and one on noise will have two landing pages (https://developer.my-org.eu/apis/air/ and https://developer.my-org.eu/apis/noise/).
The mapping between INSPIRE resources and STA resources is given below, for an example data set containing air quality.

| INSPIRE resource | STA resource | Sample path | Document reference |
| ------------- | ------------- | ------------- |-------------: |
| (Distribution<sup> 3</sup> of a) data set | Landing page | https://developer.my-org.eu/apis/addresses/ | [STA 7.2 API landing page](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_api_landing_page) |
| Data set metadata | Feature collections | https://developer.my-org.eu/apis/addresses/collections/ | [STA 7.13 Feature collections](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_collections_)
| -- | Feature collection | https://developer.my-org.eu/apis/addresses/collections/address | [STA 7.14 Feature collection](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_collection_) |
| Spatial objects | Features | https://developer.my-org.eu/apis/addresses/collections/address/items | [STA 7.15 Features](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_items_) |
| Spatial object | Feature | https://developer.my-org.eu/apis/addresses/collections/address/items/{featureId} | [STA 7.16 Feature](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_feature_) |
...
### 7.2. Requirements class “INSPIRE-pre-defined-data-set-download-STA”  <a name="req-pre-defined"></a>

| Requirements class | http://inspire.ec.europa.eu/id/spec/sta-download/1.0/req/pre-defined |
| --- | --- |
| Target type | Web API |
| Dependency | [STA Requirements class "OpenAPI 3.0"](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_requirements_class_openapi_3_0)

The Web API depends on the [STA Requirements class OpenAPI 3.0](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_requirements_class_openapi_3_0), and therefore also on the [STA Requirements class Core](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_requirements_class_core), for providing access to INSPIRE data.

**NOTE 1** The Web API shall return collections and features in the default STA coordinate reference system (WGS 84 longitude and latitude). However, the `enclosure` link for bulk download could still provide access to a data set in a different CRS.

**NOTE 2** In the STA standard, the [OpenAPI 3.0 Requirements class](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_requirements_class_openapi_3_0) is optional. This specification proposes to make it a mandatory requirement for INSPIRE in order to facilitate the development of client applications, and in particular adding support in the [European INSPIRE geoportal](https://inspire-geoportal.ec.europa.eu). That is why, the Web API SHALL comply with STA requirements class OpenAPI 3.0.
...
**NOTE** According to the STA standard a collection could also contain more than one feature type.
...
| Requirements class | http://inspire.ec.europa.eu/id/spec/sta-download/1.0/req/multilinguality |
| --- | --- |
| Target type | Web API |
| Dependency | [INSPIRE-pre-defined-data-set-download-STA](#req-pre-defined) |
...
### 7.4. Requirements class “INSPIRE-STA-GeoJSON” <a name="req-sta-json"></a>

| Requirements class | http://inspire.ec.europa.eu/id/spec/sta-download/1.0/req/geojson |
| --- | --- |
| Target type | Web API |
| Dependency | [INSPIRE-pre-defined-data-set-download-STA](#req-pre-defined)  |
| Dependency | [STA requirements class GeoJSON](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_requirements_class_geojson)  |
...
| Requirements class | http://inspire.ec.europa.eu/id/spec/sta-download/1.0/req/bulk-download |
| --- | --- |
| Target type | Web API |
| Dependency | [INSPIRE-pre-defined-data-set-download-STA](#req-pre-defined)  |
...
**EXAMPLE** Feature collections response document (adapted from [STA](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_response_4))
...
# Annex B: Mapping the requirements from the IRs to the OGC-API Features standard (and extensions) <a name="ir2sta"></a>
...
- [Recommendation 2 of STA](http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_http_1_1), regarding the support of HTTP method HEAD
